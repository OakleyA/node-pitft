...
Napi::Object FrameBufferWrapper::Init(Napi::Env env, Napi::Object exports) {
    ...
    Napi::Function func = DefineClass(
        ...
        {InstanceMethod("size", &FrameBufferWrapper::Size), InstanceMethod("data", &FrameBufferWrapper::Data),
         InstanceMethod("clear", &FrameBufferWrapper::Clear), InstanceMethod("blit", &FrameBufferWrapper::Blit),
         InstanceMethod("color", &FrameBufferWrapper::Color), InstanceMethod("fill", &FrameBufferWrapper::Fill),
         InstanceMethod("line", &FrameBufferWrapper::Line), InstanceMethod("rect", &FrameBufferWrapper::Rect),
         InstanceMethod("circle", &FrameBufferWrapper::Circle), InstanceMethod("font", &FrameBufferWrapper::Font),
         InstanceMethod("text", &FrameBufferWrapper::Text), InstanceMethod("image", &FrameBufferWrapper::Image),
...
Napi::Value FrameBufferWrapper::Data(const Napi::CallbackInfo &info) {
    ...
    Napi::Object bufferObject =
        Napi::Buffer<char>::New(env, this->frameBufferClass->fbp, this->frameBufferClass->screenSize);
...
void FrameBufferWrapper::Color(const Napi::CallbackInfo &info) {
    ...
    if (info.Length() == 1 && info[0].IsNumber())
        ...
        this->frameBufferClass->Color(info[0].As<Napi::Number>().DoubleValue(),
...
void FrameBufferWrapper::Line(const Napi::CallbackInfo &info) {
    ...
    if (info[0].IsNumber() && info[1].IsNumber() && info[2].IsNumber() && info[3].IsNumber() && info[4].IsNumber() &&
        info[5].IsNumber())
        this->frameBufferClass->Line(info[0].As<Napi::Number>().DoubleValue(), info[1].As<Napi::Number>().DoubleValue(),
                                     info[2].As<Napi::Number>().DoubleValue(), info[3].As<Napi::Number>().DoubleValue(),
                                     info[4].IsUndefined() ? 1 : info[4].As<Napi::Number>().DoubleValue());
    else
        Napi::TypeError::New(env, "invalid arguments").ThrowAsJavaScriptException();
...
void FrameBufferWrapper::Rect(const Napi::CallbackInfo &info) {
    ...
    if (info[0].IsNumber() && info[1].IsNumber() && info[2].IsNumber() && info[3].IsNumber())
        this->frameBufferClass->Rect(info[0].As<Napi::Number>().DoubleValue(), info[1].As<Napi::Number>().DoubleValue(),
                                     info[2].As<Napi::Number>().DoubleValue(), info[3].As<Napi::Number>().DoubleValue(),
...
void FrameBufferWrapper::Circle(const Napi::CallbackInfo &info) {
    ...
    if (info[0].IsNumber() && info[1].IsNumber() && info[2].IsNumber())
        this->frameBufferClass->Circle(info[0].As<Napi::Number>().DoubleValue(),
                                       info[1].As<Napi::Number>().DoubleValue(),
                                       info[2].As<Napi::Number>().DoubleValue(),
...
void FrameBufferWrapper::Font(const Napi::CallbackInfo &info) {
    ...
    if (info[0].IsString())
        this->frameBufferClass->Font(info[0].As<Napi::String>(),
...
void FrameBufferWrapper::Text(const Napi::CallbackInfo &info) {
    ...
    if (info[0].IsNumber() && info[1].IsNumber() && info[2].IsString())
        this->frameBufferClass->Text(info[0].As<Napi::Number>().DoubleValue(), info[1].As<Napi::Number>().DoubleValue(),
...
void FrameBufferWrapper::Image(const Napi::CallbackInfo &info) {
    ...
    if (info[0].IsNumber() && info[1].IsNumber() && info[2].IsString())
        this->frameBufferClass->Image(info[0].As<Napi::Number>().DoubleValue(),
